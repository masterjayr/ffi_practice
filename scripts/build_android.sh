...
# -------------------------------------------------
# Build your plugin .so for one ABI
# -------------------------------------------------
assert_so_matches_abi() {
  ...
  case "$abi" in
    arm64-v8a)
      echo "$info" | grep -qi "ELF 64-bit.*aarch64" || { echo "❌ $abi expected 64-bit aarch64. Got: $info"; exit 1; }
      ;;
    armeabi-v7a)
      echo "$info" | grep -qi "ELF 32-bit.*ARM" || { echo "❌ $abi expected 32-bit ARM. Got: $info"; exit 1; }
      ;;
    x86_64)
      echo "$info" | grep -qi "ELF 64-bit.*x86-64" || { echo "❌ $abi expected 64-bit x86-64. Got: $info"; exit 1; }
      ;;
  esac
}
...
build_plugin_so_one() {
  ...
  log "Building ffi_practice_native.so for $abi"
  ...
  cmake --build "$build_dir" --target ffi_practice_native --config Release

  local out_so="$build_dir/libffi_practice_native.so"
  [[ -f "$out_so" ]] || {
    echo "❌ Expected .so not found: $out_so"
    find "$build_dir" -maxdepth 3 -type f -name "*.so" -print
    exit 1
  }

  # ✅ critical sanity check
  assert_so_matches_abi "$abi" "$out_so"
}

copy_plugin_outputs_one() {
  local abi="$1"
  local build_dir="$BUILD_DIR/plugin-$abi"
  local so="$build_dir/libffi_practice_native.so"

  # Plugin deliverable
  local plugin_jni="$ROOT_DIR/android/src/main/jniLibs/$abi"
  mkdir -p "$plugin_jni"
  cp "$so" "$plugin_jni/libffi_practice_native.so"

  # Example app deliverable (local runs)
  local example_jni="$ROOT_DIR/example/android/app/src/main/jniLibs/$abi"
  mkdir -p "$example_jni"
  cp "$so" "$example_jni/libffi_practice_native.so"
...
main() {
  ...
  log "Plugin jniLibs: $ROOT_DIR/android/src/main/jniLibs"
  log "Example jniLibs: $ROOT_DIR/example/android/app/src/main/jniLibs"
}
